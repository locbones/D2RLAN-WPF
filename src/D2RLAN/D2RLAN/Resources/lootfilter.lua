...
local qualityMap = { [1] = "Inferior", [2] = "Normal", [3] = "Superior", [4] = "Magic", [5] = "Set", [6] = "Rare", [7] = "Unique" }
...
local colorMap = { white = "ÿc0", red = "ÿc1", green = "ÿc2", blue = "ÿc3", gold = "ÿc4", grey = "ÿc5", gray = "ÿc5", black = "ÿc6", tan = "ÿc7", orange = "ÿc8", yellow = "ÿc9", purple = "ÿc;", ["dark green"] = "ÿcA", turquoise = "ÿcN", pink = "ÿcO" }
...
local function replace_placeholders(template, Item, Me)
    ...
    -- Matches both {stat=(83)} and {stat=(83,1)}
    result = result:gsub("{stat=%((%d+),?(%d*)%)%}", function(statIndex, param)
        local idx = tonumber(statIndex)
        local prm = tonumber(param)
        local val = prm and Item:Stat(idx, prm) or Item:Stat(idx)
...
local function applyColorTagsLink(text)
    return text:gsub("{(.-)}", function(color)
        local code = colorMapLink[color:lower()]
        if code then
            return code
        else
            return "{" .. color .. "}"
            ...
            if match then
                if rule.notify then
                    if rule.notify_message then
                        ...
                        if rule.notify_link then
                            local colorKey = rule.notify_link:match("{(.-)}")
                            local linkCode = colorKey and colorMapLink[colorKey:lower()]

                            if linkCode then
                                print("Item dropped: " .. Item:Link(linkCode))
                            else
